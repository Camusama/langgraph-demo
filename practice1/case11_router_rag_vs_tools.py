"""案例 11：基于路由的 RAG / 搜索 / 计算（逐步开启版）。

目标：根据用户问题动态路由到 RAG、联网搜索或计算路径，演示多分支图的构建与工具调用。
用法：按 STEP 在 main 中逐步取消注释运行。

图结构（简述）：
- START -> route -> (retrieve -> rag_answer) 或 (llm -> tools -> llm) -> END
- route_decision：数学题 -> calc 分支；含“最新/今天/价格”等 -> search；否则按相似度选择 rag/search。
- rag_answer：用检索上下文回答并附引用。
- llm/tools：模型调用工具（搜索/计算/HTTP）后继续推理。
"""
...
def _looks_fresh(query: str) -> bool:
    lowered = query.lower()
    return (
        any(k in query for k in FRESH_KEYWORDS)
        or "today" in lowered
        or "now" in lowered
    )
