...
class Listener(stomp.ConnectionListener):
    ...
    def on_message(self, frame):
        try:
            ...
            print("order-service received:", payload["eventType"], payload["orderId"])
            # stomp.py 1.2 uses positional args for ack
            self.conn.ack(frame.headers["message-id"], frame.headers.get("subscription"))
        except Exception as exc:
            # do not ACK to allow redelivery
            print("order-service error:", exc)
...
def main():
    ...
    print("order-service listening...", SELECTOR)
    try:
        while True:
            time.sleep(1)
